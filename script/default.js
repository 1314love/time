//第一个页面停留时长
var setTimeSecond=1000;
// 表盘大小
var dialSize=10;
// 表盘位置
var dialPositionOfy=2.8;
var dialPositionOfx=1;
...
function runTime() {
	...
	for (var i = 0; i < nowValue.length; i++) {
		var num = nowValue[i];
		textSet[i][1][num].style.color = '#ff99cc'
	}
	if (isCircle) {
		var widthMid = document.body.clientWidth / dialPositionOfy;
		var heightMid = document.body.clientHeight  / dialPositionOfx;
		for (var i = 0; i < textSet.length; i++) {
			for (var j = 0; j < textSet[i][0].length; j++) {
				var r = (i + 1) * dialSize + 100 * i;
...
function initStyle() {
	...
	for (var i = 0; i < label.length; i++) {
		label[i].style.color = '#ccf'
		label[i].style.borderRadius="3px"
	}
}
